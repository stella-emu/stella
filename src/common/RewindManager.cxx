...
bool RewindManager::addState(const string& message, bool timeMachine)
{
  ...
  if(timeMachine && myStateList.currentIsValid())
  {
    ...
    {
      const uInt32 scanlines = std::max(myOSystem.console().tia().scanlinesLastFrame(), uInt32(240u));
...
{
  const Int32 scanlines = std::max(myOSystem.console().tia().scanlinesLastFrame(), uInt32(240u));
