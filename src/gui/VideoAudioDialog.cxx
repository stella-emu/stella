...
void VideoAudioDialog::addDisplayTab()
{
  ...
  myDisplayInfo = new LabelWidget(pane, ifont,
  ...
  pane->setLayout([this](GUI::BoxLayout& col) {
    ...
    GUI::alignLabels({{myRendererLbl}, {myTIAZoomLbl},
                      {myTVOverscanLbl, INDENT}, {myVSizeAdjustLbl}});
    ...
    col.addSpace(VGAP * 4);
    col.addAuto(labeledRow(myTIAZoomLbl, myTIAZoom));
    col.addSpace(VGAP);
...
#endif
    col.addSpace(VGAP);
    col.addAuto(labeledRow(myTVOverscanLbl, myTVOverscan, 0, INDENT));
    col.addSpace(VGAP * 4);
    col.addAuto(anchoredItem(myCorrectAspect));
    col.addSpace(VGAP);
    col.addAuto(labeledRow(myVSizeAdjustLbl, myVSizeAdjust));
...
void VideoAudioDialog::addPaletteTab()
{
  ...
  myTVHue      = createCustomSlider(pane, wid, myTVHueLbl, "Hue", Cmd::PaletteUpdated);
  myTVSatur    = createCustomSlider(pane, wid, myTVSaturLbl, "Saturation", Cmd::PaletteUpdated);
  myTVContrast = createCustomSlider(pane, wid, myTVContrastLbl, "Contrast", Cmd::PaletteUpdated);
  myTVBright   = createCustomSlider(pane, wid, myTVBrightLbl, "Brightness", Cmd::PaletteUpdated);
  myTVGamma    = createCustomSlider(pane, wid, myTVGammaLbl, "Gamma", Cmd::PaletteUpdated);
  ...
  pane->setLayout([this](GUI::BoxLayout& col) {
    ...
    GUI::alignLabels({{myTIAPaletteLbl}, {myPhaseShiftLbl, INDENT},
                      {myTVHueLbl}, {myTVSaturLbl}, {myTVContrastLbl},
                      {myTVBrightLbl}, {myTVGammaLbl}});
    // The R/G/B saturation sliders are a column of their own (the shift sliders
    // beside them have no label at all)
    GUI::alignLabels({{myTVRedScaleLbl}, {myTVGreenScaleLbl}, {myTVBlueScaleLbl}});
    ...
    GUI::alignTracks({myTVRedScale, myTVRedShift}, {myTVRedScaleLbl, nullptr},
                     rgbSpan, fontWidth);
    GUI::alignTracks({myTVGreenScale, myTVGreenShift}, {myTVGreenScaleLbl, nullptr},
                     rgbSpan, fontWidth);
    GUI::alignTracks({myTVBlueScale, myTVBlueShift}, {myTVBlueScaleLbl, nullptr},
                     rgbSpan, fontWidth);

    const auto rgbRow = [&](LabelWidget* scaleLabel, SliderWidget* scale,
                            SliderWidget* shift) {
    ...
    controls->addAuto(labeledRow(myTIAPaletteLbl, myTIAPalette));
    controls->addSpace(VGAP);
    controls->addAuto(labeledRow(myPhaseShiftLbl, myPhaseShift, 0, INDENT));
    controls->addSpace(VGAP);
    controls->addAuto(rgbRow(myTVRedScaleLbl, myTVRedScale, myTVRedShift));
    controls->addSpace(VGAP);
    controls->addAuto(rgbRow(myTVGreenScaleLbl, myTVGreenScale, myTVGreenShift));
    controls->addSpace(VGAP);
    controls->addAuto(rgbRow(myTVBlueScaleLbl, myTVBlueScale, myTVBlueShift));
    controls->addSpace(VGAP * 2);
    controls->addAuto(labeledRow(myTVHueLbl, myTVHue));
    controls->addSpace(VGAP);
    controls->addAuto(labeledRow(myTVSaturLbl, myTVSatur));
    controls->addSpace(VGAP);
    controls->addAuto(labeledRow(myTVContrastLbl, myTVContrast));
    controls->addSpace(VGAP);
    controls->addAuto(labeledRow(myTVBrightLbl, myTVBright));
    controls->addSpace(VGAP);
    ...
    detect->addAuto(anchoredItem(myAutodetectLbl));
...
void VideoAudioDialog::addTVEffectsTab()
{
  ...
  // Which standard's adjustables the layout below shows.  Both widget sets
  // are always built (below) and handleTVModeChange() still shows/hides them
  // by Custom-mode state the usual way; this only decides which one the
  // layout gives room to, since the other is never applicable for the
  // running console and would otherwise just leave a gap where it was hidden.
  ...
  // Custom adjustables.  NTSC (5 adjustables) and PAL (2 adjustables + drift
  // + colour loss) each get their own widget set; handleTVModeChange() shows
  // only the one that applies.  SECAM has no adjustables at all.
  myNTSCSharp     = createCustomSlider(pane, wid, myNTSCSharpLbl, "Sharpness", GuiCmd::None);
  myNTSCRes       = createCustomSlider(pane, wid, myNTSCResLbl, "Resolution", GuiCmd::None);
  myNTSCArtifacts = createCustomSlider(pane, wid, myNTSCArtifactsLbl, "Artifacts", GuiCmd::None);
  myNTSCFringe    = createCustomSlider(pane, wid, myNTSCFringeLbl, "Fringing", GuiCmd::None);
  myNTSCBleed     = createCustomSlider(pane, wid, myNTSCBleedLbl, "Bleeding", GuiCmd::None);

  myPALSharp = createCustomSlider(pane, wid, myPALSharpLbl, "Sharpness", GuiCmd::None);
  myPALBlend = createCustomSlider(pane, wid, myPALBlendLbl, "Blend", GuiCmd::None);

  // Subcarrier drift.  Not a TV adjustment like the two sliders above: it
  // is a property of the PAL console, whose subcarrier and pixel clock come
  // from two independent crystals, so the artifact pattern never sits still.
  // In hundredths of a Hz, since the nominal beat is 1.25 Hz.
  myPALDriftLbl = new LabelWidget(pane, _font, "Drift");
  myPALDrift = new SliderWidget(pane, _font, 1, Cmd::PALDriftChanged, 6);
  ...
  myPALColorLossLbl = new LabelWidget(pane, _font, "Color loss");
  myPALColorLoss = new PopUpWidget(pane, _font, items);
  ...
  myTVPhosLevel = createCustomSlider(pane, wid, myTVPhosLevelLbl, "Blend",
                                     Cmd::PhosphorBlendChanged);
  ...
  myTVCurvature = createCustomSlider(pane, wid, myTVCurvatureLbl, "Curvature",
                                     Cmd::CurvatureChanged);
  ...
  myTVCurvatureY = createCustomSlider(pane, wid, myTVCurvatureYLbl, "Vertical",
                                      Cmd::CurvatureYChanged);
  ...
  myTVScanIntense = createCustomSlider(pane, wid, myTVScanIntenseLbl, "Intensity",
                                       Cmd::ScanlinesChanged);
  ...
  myTVScanMaskLbl = new LabelWidget(pane, _font, "Mask");
  ...
  auto createCloneButton = [&](string_view desc, GuiCmd::Code cmd) {
    auto* button = new ButtonWidget(pane, _font, desc, cmd);
    wid.push_back(button);
    return button;
  };

  myCloneRGB       = createCloneButton("Clone RGB", Cmd::CloneRGB);
  myCloneSvideo    = createCloneButton("Clone S-Video", Cmd::CloneSvideo);
  myCloneComposite = createCloneButton("Clone Composite", Cmd::CloneComposite);
  myCloneCustom    = createCloneButton("Revert", Cmd::CloneCustom);

  addToFocusList(wid, myTab, tabID);
  pane->setHelpAnchor("VideoAudioEffects");

  pane->setLayout([this, isNTSC, isPAL](GUI::BoxLayout& col) {
    using GUI::BoxLayout;
    using GUI::anchoredItem;
    using GUI::labeledRow;
    using GUI::indentedFill;
    ...
    // The two pop-ups read as one column and must END at the same place, so
    // they share both a label column and a box width.
    ...
    // group lets alignTracks() cross that gap itself.  NTSC and PAL each have
    // their own widget set here; only the one that applies to the running
    // console is given a row, so the other doesn't leave a gap in its place.
    auto modes = std::make_unique<BoxLayout>(Dir::Vertical);
    modes->addAuto(labeledRow(myTVModeLbl, myTVMode));
    modes->addSpace(VGAP);

    if(isNTSC)
    {
      GUI::alignLabels({{myNTSCSharpLbl}, {myNTSCResLbl}, {myNTSCArtifactsLbl},
                        {myNTSCFringeLbl}, {myNTSCBleedLbl}, {myTVPhosLevelLbl},
                        {myTVScanIntenseLbl}});
      GUI::alignTracks({myNTSCSharp, myNTSCRes, myNTSCArtifacts, myNTSCFringe,
                        myNTSCBleed, myTVPhosLevel, myTVScanIntense},
                       myTVMode, INDENT, myNTSCSharpLbl, myTVModeLbl);

      modes->addAuto(labeledRow(myNTSCSharpLbl, myNTSCSharp, 0, INDENT));
      modes->addSpace(VGAP);
      modes->addAuto(labeledRow(myNTSCResLbl, myNTSCRes, 0, INDENT));
      modes->addSpace(VGAP);
      modes->addAuto(labeledRow(myNTSCArtifactsLbl, myNTSCArtifacts, 0, INDENT));
      modes->addSpace(VGAP);
      modes->addAuto(labeledRow(myNTSCFringeLbl, myNTSCFringe, 0, INDENT));
      modes->addSpace(VGAP);
      modes->addAuto(labeledRow(myNTSCBleedLbl, myNTSCBleed, 0, INDENT));
    }
    else if(isPAL)
    {
      GUI::alignLabels({{myPALSharpLbl}, {myPALBlendLbl}, {myPALDriftLbl},
                        {myPALColorLossLbl}, {myTVPhosLevelLbl},
                        {myTVScanIntenseLbl}});
      GUI::alignTracks({myPALSharp, myPALBlend, myPALDrift, myTVPhosLevel,
                        myTVScanIntense}, myTVMode, INDENT,
                       myPALSharpLbl, myTVModeLbl);

      modes->addAuto(labeledRow(myPALSharpLbl, myPALSharp, 0, INDENT));
      modes->addSpace(VGAP);
      modes->addAuto(labeledRow(myPALBlendLbl, myPALBlend, 0, INDENT));
      modes->addSpace(VGAP);
      modes->addAuto(labeledRow(myPALDriftLbl, myPALDrift, 0, INDENT));
      modes->addSpace(VGAP);
      modes->addAuto(labeledRow(myPALColorLossLbl, myPALColorLoss, 0, INDENT));
    }
    else
    {
      GUI::alignLabels({{myTVPhosLevelLbl}, {myTVScanIntenseLbl}});
      GUI::alignTracks({myTVPhosLevel, myTVScanIntense}, myTVMode, INDENT,
                       myTVPhosLevelLbl, myTVModeLbl);
    }
    ...
    GUI::alignButtons({myCloneRGB, myCloneSvideo, myCloneComposite, myCloneCustom});
    auto clones = std::make_unique<BoxLayout>(Dir::Vertical, VGAP);
    for(auto* b: {myCloneRGB, myCloneSvideo, myCloneComposite, myCloneCustom})
    ...
    // Curvature is its own group: "Curvature" is the section head (unindented,
    // like "Scanlines:" below), "Vertical" is subordinate to it and sits a
    // level in.  Both still end flush with the TV mode pop-up, via the same
    // cross-group naming used for the standard's adjustables above.
    GUI::alignLabels({{myTVCurvatureLbl, 0}, {myTVCurvatureYLbl, INDENT}});
    GUI::alignTracks({myTVCurvature, myTVCurvatureY}, myTVMode, 0,
                     myTVCurvatureLbl, myTVModeLbl);

    // The mask pop-up is on its own, and a control in no group gets no clearance
    // between its label and its box
    GUI::alignLabels({{myTVScanMaskLbl}});
    ...
    scanRow->addStretch(labeledRow(myTVScanMaskLbl, myTVScanMask, 0, 0, true));

    col.addAuto(std::move(main));
    col.addSpace(VGAP * 4);
    col.addAuto(labeledRow(myTVPhosphorLbl, myTVPhosphor));
    col.addSpace(VGAP);
    col.addAuto(labeledRow(myTVPhosLevelLbl, myTVPhosLevel, 0, INDENT));
    col.addSpace(VGAP * 2);
    col.addAuto(labeledRow(myTVCurvatureLbl, myTVCurvature));
    col.addSpace(VGAP);
    col.addAuto(labeledRow(myTVCurvatureYLbl, myTVCurvatureY, 0, INDENT));
    col.addSpace(VGAP * 2);
    col.addAuto(anchoredItem(myTVScanLbl));
    col.addSpace(VGAP);
...
void VideoAudioDialog::addBezelTab()
{
  ...
  myWinLeftSliderLbl = new LabelWidget(pane, _font, "Left");
  myWinLeftSlider = winSlider();
  myWinRightSliderLbl = new LabelWidget(pane, _font, "Right");
  myWinRightSlider = winSlider();
  myWinTopSliderLbl = new LabelWidget(pane, _font, "Top");
  myWinTopSlider = winSlider();
  myWinBottomSliderLbl = new LabelWidget(pane, _font, "Bottom");
  ...
  pane->setLayout([this](GUI::BoxLayout& col) {
    ...
    GUI::alignLabels({{myWinLeftSliderLbl}, {myWinRightSliderLbl},
                      {myWinTopSliderLbl}, {myWinBottomSliderLbl}});
    ...
    col.addAuto(labeledRow(myWinLeftSliderLbl, myWinLeftSlider, 0, INDENT * 2, true));
    col.addSpace(VGAP);
    col.addAuto(labeledRow(myWinRightSliderLbl, myWinRightSlider, 0, INDENT * 2, true));
    col.addSpace(VGAP);
    col.addAuto(labeledRow(myWinTopSliderLbl, myWinTopSlider, 0, INDENT * 2, true));
    col.addSpace(VGAP);
    col.addAuto(labeledRow(myWinBottomSliderLbl, myWinBottomSlider, 0, INDENT * 2, true));
  });
}
...
void VideoAudioDialog::addAudioTab()
{
  ...
  myFreqPopupLbl = new LabelWidget(pane, _font, "Sample rate");
  ...
  // Stereo sound
  myStereoSoundCheckbox = new CheckboxWidget(pane, _font, "Stereo for all ROMs");
  ...
  pane->setLayout([this](GUI::BoxLayout& col) {
    ...
    GUI::alignTracks({myDpcPitch}, myModePopup, 0, myDpcPitchLbl, myModePopupLbl);

    // They sit a level further in than Mode, so the cell that reaches its
    // right edge is that much narrower
    const int flushWidth = GUI::flushSpan(myModePopup, myModePopupLbl, INDENT);
    ...
    modeRow->addStretch(labeledRow(myModePopupLbl, myModePopup));
    col.addAuto(std::move(modeRow));
    col.addSpace(VGAP);
    auto freqRow = std::make_unique<BoxLayout>(Dir::Horizontal);
    freqRow->addSpace(INDENT * 2);
    freqRow->addFixed(labeledRow(myFreqPopupLbl, myFreqPopup, 0, 0, true), flushWidth);
    col.addAuto(std::move(freqRow));
    col.addSpace(VGAP);
    auto resamplingRow = std::make_unique<BoxLayout>(Dir::Horizontal);
    resamplingRow->addSpace(INDENT * 2);
    resamplingRow->addFixed(labeledRow(myResamplingPopupLbl, myResamplingPopup, 0, 0, true), flushWidth);
    col.addAuto(std::move(resamplingRow));
    col.addSpace(VGAP);
    col.addAuto(labeledRow(myHeadroomSliderLbl, myHeadroomSlider, 0, INDENT * 2));
    col.addSpace(VGAP);
    col.addAuto(labeledRow(myBufferSizeSliderLbl, myBufferSizeSlider, 0, INDENT * 2));
    col.addSpace(VGAP);
    col.addAuto(anchoredItem(myStereoSoundCheckbox));
    col.addSpace(VGAP);
    col.addAuto(labeledRow(myDpcPitchLbl, myDpcPitch, 0, INDENT));
  });
}
...
void VideoAudioDialog::loadConfig()
{
  ...
  handleTVModeChange(static_cast<TVMode>(preset));
...
void VideoAudioDialog::handleTVModeChange(TVMode preset)
{
  ...
  // when inapplicable, keeping the tab clean.  SECAM has no adjustables at
  // all — but it does have connection types, so the mode itself stays
  // selectable: for SECAM they choose the chroma bandwidth, and Disabled is
  // what turns the delay line off and drops the picture back to the flat
  // 8-entry palette.
  const auto showWidget = [](Widget* label, Widget* w, bool visible, bool editable)
  {
  ...
  showWidget(myNTSCSharpLbl, myNTSCSharp, isNTSC, enable && isNTSC);
  showWidget(myNTSCResLbl, myNTSCRes, isNTSC, enable && isNTSC);
  showWidget(myNTSCArtifactsLbl, myNTSCArtifacts, isNTSC, enable && isNTSC);
  showWidget(myNTSCFringeLbl, myNTSCFringe, isNTSC, enable && isNTSC);
  showWidget(myNTSCBleedLbl, myNTSCBleed, isNTSC, enable && isNTSC);

  showWidget(myPALSharpLbl, myPALSharp, isPAL, enable && isPAL);
  showWidget(myPALBlendLbl, myPALBlend, isPAL, enable && isPAL);
  ...
  showWidget(myPALDriftLbl, myPALDrift, isPAL, composite);
  showWidget(myPALColorLossLbl, myPALColorLoss, isPAL, composite);

  const bool showClones = !isSECAM;
  for(auto* b: {myCloneComposite, myCloneSvideo, myCloneRGB, myCloneCustom})
  {
    b->setVisible(showClones);
...
#endif
  myTVOverscanLbl->setEnabled(enable);
...
void VideoAudioDialog::handlePhosphorChange()
{
  ...
  myTVPhosLevelLbl->setEnabled(enable);
...
void VideoAudioDialog::handleCurvatureChange()
{
  ...
  myTVCurvatureYLbl->setEnabled(curved);
...
void VideoAudioDialog::handleBezelChange()
{
  ...
  myWinLeftSliderLbl->setEnabled(enable && nonAuto);
  myWinLeftSlider->setEnabled(enable && nonAuto);
  myWinRightSliderLbl->setEnabled(enable && nonAuto);
  myWinRightSlider->setEnabled(enable && nonAuto);
  myWinTopSliderLbl->setEnabled(enable && nonAuto);
  myWinTopSlider->setEnabled(enable && nonAuto);
  myWinBottomSliderLbl->setEnabled(enable && nonAuto);
                                     ...
                                     int data, int id)
{
  switch(cmd)
  {
    ...
    case Cmd::TvModeChanged:
      handleTVModeChange(static_cast<TVMode>(myTVMode->getSelectedTag().toInt()));
      break;

    case Cmd::CloneComposite: loadTVAdjustables(TVMode::Composite);
      break;
    case Cmd::CloneSvideo: loadTVAdjustables(TVMode::SVideo);
      break;
    case Cmd::CloneRGB: loadTVAdjustables(TVMode::RGB);
      break;
    case Cmd::CloneCustom: loadTVAdjustables(TVMode::Custom);
      break;
    ...
    case Cmd::ScanlinesChanged:
      if(myTVScanIntense->getValue() == 0)
      {
        ...
        myTVScanMaskLbl->setEnabled(false);
        myTVScanMask->setEnabled(false);
      }
      else
      {
        myTVScanIntense->setValueUnit("%");
        myTVScanMaskLbl->setEnabled(true);
        myTVScanMask->setEnabled(true);
      }
      break;

    case Cmd::CurvatureChanged:
    case Cmd::CurvatureYChanged:
      handleCurvatureChange();
      break;

    case Cmd::PALDriftChanged:
      handlePALDriftChange();
      break;
...
void VideoAudioDialog::updateAudioEnabledState()
{
  ...
  const bool dpcEnable = active && instance().hasConsole() &&
      instance().console().cartridge().name() == "CartridgeDPC";
  myDpcPitchLbl->setEnabled(dpcEnable);
  myDpcPitch->setEnabled(dpcEnable);

  myFreqPopupLbl->setEnabled(active && userMode);
  myFreqPopup->setEnabled(active && userMode);
  myResamplingPopupLbl->setEnabled(active && userMode);
  myResamplingPopup->setEnabled(active && userMode);
  myHeadroomSliderLbl->setEnabled(active && userMode);
  myHeadroomSlider->setEnabled(active && userMode);
  myBufferSizeSliderLbl->setEnabled(active && userMode);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// A custom-adjustable slider: 0-100%, in 1% steps, paired with its own
// LabelWidget (returned via 'lbl').  The track width is the dialog's choice
// and is set when the tab lays itself out.
SliderWidget*
VideoAudioDialog::createCustomSlider(TabPaneWidget* pane, WidgetArray& wid,
                                     LabelWidget*& lbl, string_view desc,
                                     GuiCmd::Code cmd)
{
  lbl = new LabelWidget(pane, _font, desc);

  auto* slider = new SliderWidget(pane, _font, 1, cmd, 4, "%");
