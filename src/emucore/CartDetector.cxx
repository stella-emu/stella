...
bool CartDetector::isProbablyARM(const ByteBuffer& image, uInt32 size)
{
  ...
  if(searchForBytes(image.get(), std::min(size, uInt32(1024u)), signature[0], 4, 1))
    return true;
  else
    return searchForBytes(image.get(), std::min(size, uInt32(1024u)), signature[1], 4, 1);
}
...
bool CartDetector::isProbablyMDM(const ByteBuffer& image, uInt32 size)
{
  ...
  uInt8 mdmc[] = { 'M', 'D', 'M', 'C' };
  return searchForBytes(image.get(), std::min(size, uInt32(8192u)), mdmc, 4, 1);
}
