...
void Driving::updateControllerAxes()
{
  ...
  if(myEvent.get(myCCWEvent) != 0 && myLastCCWEvent == 0)
    myCounterHires = ((myGrayIndex + 4) * COUNTER_SCALE) / SENSITIVITY - 1; // set to end of previous counter interval
  else if(myEvent.get(myCWEvent) != 0 && myLastCWEvent == 0)
    myCounterHires = ((myGrayIndex + 1) * COUNTER_SCALE) / SENSITIVITY;     // set to begin of next counter interval
...
void Driving::updateStelladaptorAxes()
{
  ...
  {
    myLastYaxis = yaxis;
    if(yaxis <= -16384 - SA_Threshold)      myGrayIndex = 3; // up
    else if(yaxis > 16384 + SA_Threshold)   myGrayIndex = 1; // down
    else if(yaxis >= 16384 - SA_Threshold)  myGrayIndex = 2; // up + down
    else                                    myGrayIndex = 0; // no movement
    ...
    myCounter = (myGrayIndex + 0.5F) * 4.F / SENSITIVITY;
    myCounterHires = myCounter * 64;
  }
}
