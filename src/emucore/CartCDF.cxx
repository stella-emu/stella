    ...
    myFractionalClocks(0.0)
{
  ...
  myThumbEmulator = make_unique<Thumbulator>
      (reinterpret_cast<uInt16*>(myImage),
       reinterpret_cast<uInt16*>(myCDFRAM),
...
{
  uInt8* ptr = myCDFRAM + 2048;
  if(mySettings.getBool("ramrandom"))
    for(uInt32 i = 0; i < 8192-2048; ++i)
      ptr[i] = mySystem->randGenerator().next();
  else
    memset(ptr, 0, 8192-2048);
...
void CartridgeCDF::install(System& system)
{
  ...
  for(uInt16 i = 0x1000; i < 0x1040; i += (1 << shift))
...
bool CartridgeCDF::bank(uInt16 bank)
{
  ...
  for(uInt16 addr = 0x1040; addr < 0x2000; addr += (1 << shift))
  {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
uInt16 CartridgeCDF::bank() const
{
